...
class DiscriminationLoss:
    ...
    def measurement_to_loss(self, label: int, measurement: List):
        # 1 == a, 0 == b
        measurement = tf.cast(tf.squeeze(measurement), tf.int32)
        fn_a = lambda: tf.cast(tf.equal(measurement, [1, -1]), dtype=tf.float32)
        fn_b = lambda: tf.cast(tf.logical_or(tf.equal(measurement, [1, 1]), tf.equal(measurement, [-1, 1])), dtype=tf.float32)
